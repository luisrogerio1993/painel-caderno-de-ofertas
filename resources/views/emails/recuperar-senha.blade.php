@extends('emails.template')
@section('conteudo')
<p>Olá {{ $name }}, você está recebendo este e-mail porque recebemos um pedido de redefinição de senha para sua conta.</p>
<p>Se você fez o pedido clique no botão baixo e siga os passos pedidos no painel.</p>
<p style="text-align: center;">
    <a href="{{ route('site.home') }}/password/reset/{{$token}}" class="botao-email">Redefinir senha</a>
</p>
<div class="row div-ajuda-link">
    <p>
        Se você estiver tendo problemas para clicar no botão "Acessar painel", copie e cole o URL abaixo em seu navegador:<br/>
        {{ route('site.home') }}/password/reset/{{$token}}
    </p>
    <p>
        Se você não solicitou uma reinicialização da senha, nenhuma ação adicional será necessária.
    </p>
</div>
@stop